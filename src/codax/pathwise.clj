...
(def date-time-formatter (time-format/formatter :basic-date-time))

(defn date-time? [x]
  (instance? org.joda.time.DateTime x))

(defn encode-date-time [d]
  (time-format/unparse date-time-formatter d))
