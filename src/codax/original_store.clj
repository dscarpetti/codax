...
(defn get-node [txn address]
  (if (nil? address)
    ...
    (or
     ...
     (if-let [item (get (:cache @(:data (:db txn))) address)]
       item
       (let [item (load-node txn address)]
         (swap! (:data (:db txn)) update :cache assoc address item)
         item)))))
...
(defn make-node [txn type records & [old-address]]
  (let [address (promise);(inc (:address txn))
        node {:type type
              :id address
              :records (if (instance? clojure.lang.PersistentTreeMap records)
                         records
                         (do ;(println "converting")
...
(defn address-node [fulfillments promise type records]
  (let [node {:type type :records records}
    ...
    (swap! (:db-data fulfillments) update :cache assoc current-offset node)
    (-> fulfillments
...
(defn seek-vals [results txn {:keys [records] :as node} start-key end-key limit rev ssfn]
  (if (and limit (<= limit (count results)))
    (subvec results 0 limit)
    (if (leaf-node? node)
      (reduce conj results (ssfn records));(if rev
;                             (rsubseq records >= start-key <= end-key)
 ;                            (subseq records >= start-key <= end-key)))
      (reduce (fn [res [_ id]]
                (seek-vals res txn (get-node txn id) start-key end-key limit rev ssfn))
              results
              (get-sub-seek records start-key end-key rev)))))

(defn b+seek [txn start end & {:keys [limit reverse]}]
  (let [root (get-node txn (:root-id txn))]
    (seek-vals [] txn root start end limit reverse (if reverse
                                                     #(rsubseq % >= start <= end)
                                                     #(subseq % >= start <= end)
                                                     ))))
  ...
  (locking (:lock-obj db)
    (let [origin-path (:filepath db)
      ...
      (with-compaction-lock [db]
        (let [compaction-timer @(:compaction-timer db)
              current-time (System/currentTimeMillis)]
          (println
           "     compaction " (pad-num  (:count compaction-timer))
           "     cache Size " (pad-num (count (:cache @(:data db))))
           "     ops/sec " (pad-num (int (/ 1000 (/ (- current-time (:last compaction-timer)) 1000))))
           "     seconds elapsed " (pad-num (int (/ (- current-time (:last compaction-timer)) 1000)))
           "     minutes open " (pad-num (int (/ (- current-time (:started compaction-timer)) 60000))))
          (swap! (:compaction-timer db) assoc :last  current-time :count (inc (:count compaction-timer))))
...
(defn stress-database []
  (let [db (open-database "data/crash-test-dummy")
        ...
        compacts nil;(repeat 50 #(compact-database db))
